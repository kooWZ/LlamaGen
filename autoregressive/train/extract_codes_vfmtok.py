...
vfmtok_path = os.path.abspath(
    os.path.join(llamagen_path, "external_tokenizers/baseline")
)
sys.path.append(vfmtok_path)
from baseline import build_and_load_model
...
def main(args):
    ...
    _, vq_model = build_and_load_model(args.config_path, args.ckpt_path)
    ...
    for p, x, y in progress_bar:
        ...
        if index > last_index:
            ...
            with torch.no_grad():
                (z_quant, _), _, indices = vq_model.encode(x)

            codes = indices.reshape(bsz, n_aug, -1)
            z_indices = codes.flatten(0, 1)

            
            samples = vq_model.decode_codes_to_img(z_indices, 256)
            samples = samples.cpu().numpy()
            saveDir = 'images'
            os.makedirs(saveDir, exist_ok=True)
            for j, sample in enumerate(samples):
                Image.fromarray(sample).save(f'{saveDir}/delete_{j}.png')
                print("saved")

            exit()
            encoded_tokens = indices.view(bsz, n_aug, -1).cpu().numpy()
            for i in range(bsz):
                x = encoded_tokens[i]
                x_concat.append(x)
                y_concat.append(
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--data-path",
        type=str,
        default="/root/projects/continuous_tokenizer/ImageNet/fakeval",
    )
    parser.add_argument(
        "--config-path",
        type=str,
        default="/root/projects/continuous_tokenizer/configs/vfmtok_transformer.yaml",
    )
    parser.add_argument(
        "--ckpt-path",
        type=str,
        default="/root/projects/continuous_tokenizer/weights/vfmtok-tokenizer.pt",
    )
    parser.add_argument(
        "--code-path",
        type=str,
        default="/root/projects/continuous_tokenizer/LlamaGen/dataset/ImageNet-1k/baseline_codes/",
    )
    parser.add_argument("--dataset", type=str, default="imagenet")
    parser.add_argument(
        "--image-size", type=int, choices=[256, 336 ,384, 448, 512], default=336
    )
    parser.add_argument(
